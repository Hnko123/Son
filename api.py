...
ORDERS_SOCKET_EVENT = "orders:update"
TASKS_SOCKET_EVENT = "tasks:update"
CHAT_MESSAGE_EVENT = "chat:message"
CHAT_TYPING_EVENT = "chat:typing"
CHAT_READ_EVENT = "chat:read"
PRESENCE_UPDATE_EVENT = "presence:update"
HEALTH_PING_EVENT = "health:ping"
HEALTH_PONG_EVENT = "health:pong"
active_socket_users: Dict[str, int] = {}
active_socket_user_meta: Dict[str, Dict[str, Any]] = {}
...
async def emit_chat_message(message: Dict[str, Any]):
    try:
        await sio.emit(CHAT_MESSAGE_EVENT, {"message": message})
    except Exception as exc:
        logger.debug(f"[WS] Failed to emit chat message: {exc}")


async def emit_presence_update():
    try:
        with SessionLocal() as session:
            payload = collect_online_users(session)
        await sio.emit(PRESENCE_UPDATE_EVENT, {"users": payload})
    except Exception as exc:
        logger.debug(f"[WS] Failed to emit presence update: {exc}")
...
@sio.event
async def chat_typing(sid, payload: Optional[Dict[str, Any]] = None):
    meta = active_socket_user_meta.get(sid)
    if not meta:
        return
    payload = payload or {}
    try:
        await sio.emit(
            CHAT_TYPING_EVENT,
            {
                ...
                "recipient_id": payload.get("recipient_id") if payload else None,
                "timestamp": datetime.utcnow().isoformat(),
            },
            skip_sid=sid
        )
    except Exception as exc:
        logger.debug(f"[WS] chat_typing emit failed: {exc}")


@sio.event
async def chat_read(sid, payload: Optional[Dict[str, Any]] = None):
    meta = active_socket_user_meta.get(sid)
    if not meta:
        return
    payload = payload or {}
    try:
        ...
        logger.debug(f"[WS] chat_read emit failed: {exc}")


@sio.event
async def health_ping(sid, payload: Optional[Dict[str, Any]] = None):
    try:
        await sio.emit(
            HEALTH_PONG_EVENT,
            {
        ...
        logger.debug(f"[WS] Failed to respond to health ping ({sid}): {exc}")
...
def ping_online(
    ...
    try:
        ...
        _cleanup_stale_presence(db)
        db.commit()
    asyncio.create_task(emit_presence_update())
    return {
        "status": "ok",
        "session_id": session_id,
...
def collect_online_users(session: Session) -> List[Dict[str, Any]]:
    cutoff = datetime.utcnow() - ONLINE_ACTIVE_WINDOW
    presence_rows = (
        session.query(UserPresence)
...
def get_online_users(
    ...
    return collect_online_users(db)
...
async def edit_order_fields(
    ...
    await emit_orders_update("order_updated", {"transaction": transaction, "fields": updated_fields})
...
async def create_task(
    ...
    await emit_tasks_update("task_created", task_response.dict())
...
async def update_task(
    ...
    await emit_tasks_update("task_updated", updated_response.dict())
...
async def delete_task(
    ...
    db.delete(task)
    db.commit()
    await emit_tasks_update("task_deleted", {"id": task_id})
...
async def send_chat_message(
    ...
    asyncio.create_task(emit_chat_message(msg))
...
async def upload_chat_image(
    ...
    try:
        ...
        save_chat_messages(chat_messages)
        asyncio.create_task(emit_chat_message(image_message))
...
async def create_manual_order(
    ...
    await emit_orders_update("manual_created", order)
    return order
...
async def update_manual_order(
    ...
    await emit_orders_update("manual_updated", order)
    return order
...
async def delete_manual_order(
    ...
    await emit_orders_update("manual_deleted", {"__manualId": manual_id})
    return {"success": True}
